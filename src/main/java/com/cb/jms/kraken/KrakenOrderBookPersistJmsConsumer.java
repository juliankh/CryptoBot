...
public class KrakenOrderBookPersistJmsConsumer extends AbstractJmsConsumer {
    ...
    protected void customProcess(byte[] payload) {
        KrakenBatch<XchangeKrakenOrderBook> krakenBatch = TimeUtils.runTimedCallable_ObjectOutput(() -> SerializationUtils.deserialize(payload), "Deserializing Kraken OrderBook Jms payload");
        CurrencyPair batchCurrencyPair = krakenBatch.getCurrencyPair();
